...
pilot_audit/
    ...
    figures/
        ... PNG reality-check figures, including discrete player-count and coverage plots ...
...
def save_player_count_distribution(
    label: str, participant_counts: pd.Series, figures: Path
) -> None:
    """
    Observed-match counts are a discrete integer variable, so a histogram with
    continuous bins (especially after changing the x-axis to log scale) can hide
    the structure. Use exact integer frequencies instead.

    The y-axis is logarithmic because the one-match group is usually orders of
    magnitude larger than the long-history tail.
    """
    if len(participant_counts) == 0:
        return

    freq = participant_counts.astype(int).value_counts().sort_index()
    repeat_pct = safe_pct(int((participant_counts >= 2).sum()), len(participant_counts))

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.bar(freq.index.astype(int), freq.values)
    ax.set_yscale("log")
    ax.set_xlabel("Observed matches per player")
    ax.set_ylabel("Number of players (log scale)")
    ax.set_title(
        f"{label}: most pilot players appear once; "
        f"{repeat_pct:.1f}% appear at least twice"
    )

    # Integer ticks keep this explicitly discrete without inventing bin cutoffs.
    max_count = int(freq.index.max())
    if max_count <= 30:
        ax.set_xticks(range(1, max_count + 1))
    else:
        # Avoid unreadable tick labels for an unexpectedly long tail.
        ticks = sorted(set([1, 2, 3, 5, 10, 20, max_count]))
        ax.set_xticks(ticks)
...
def save_player_coverage_curve(
    label: str, participant_counts: pd.Series, figures: Path
) -> None:
    """
    Feasibility-oriented survival/coverage curve:
        y(k) = number of players with at least k observed matches.

    This is more informative than a histogram for our longitudinal question,
    because it directly shows how many players remain usable as the required
    history length increases.
    """
    if len(participant_counts) == 0:
        return

    max_count = int(participant_counts.max())
    ks = np.arange(1, max_count + 1)
    coverage = np.array([(participant_counts >= k).sum() for k in ks], dtype=int)

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.step(ks, coverage, where="post")
    ax.set_yscale("log")
    ax.set_xlabel("Minimum number of observed matches required (k)")
    ax.set_ylabel("Players with at least k matches (log scale)")
    ax.set_title(
        f"{label}: longitudinal player coverage declines as history requirements increase"
    )
    ax.set_xticks(ks if max_count <= 20 else sorted(set([1, 2, 3, 5, 10, 20, max_count])))
    fig.tight_layout()
    fig.savefig(figures / f"{label}_player_coverage_curve.png", dpi=160)
    plt.close(fig)
...
def audit_one(label: str, root: Path, output: Path) -> Dict[str, object]:
    ...
    save_player_count_distribution(label, player_counts, figures)
    save_player_coverage_curve(label, player_counts, figures)
