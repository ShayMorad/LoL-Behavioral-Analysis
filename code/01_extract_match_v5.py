...
Phase 1 data-engineering pipeline for the Data Science 67978 LoL project.
...
# ---------------------------------------------------------------------------
# Tracked/seed-player resolution
# ---------------------------------------------------------------------------

def normalize_riot_id(game_name: Any, tag_line: Any) -> Optional[str]:
    """
    Normalize a Riot ID for matching only.

    We deliberately do not persist names in the processed analytical tables.
    NFKC + casefold handles harmless Unicode/case differences while preserving
    the semantic "gameName#tagLine" structure.
    """
...
def load_seed_lists(named_paths: List[Tuple[str, Path]]) -> Tuple[
    Dict[str, set[str]], Dict[str, set[str]]
]:
    """
    Returns:
        seed_index: normalized Riot ID -> set of seed-list labels
        seeds_by_label: label -> normalized Riot IDs

    The seed files contain Riot IDs, not PUUIDs. During the raw JSON scan we
    resolve those aliases to the stable PUUID-derived player_id. This lets us
    later distinguish explicitly crawled players from incidental opponents /
    teammates without storing display names.
    """
    seed_index: Dict[str, set[str]] = defaultdict(set)
    seeds_by_label: Dict[str, set[str]] = defaultdict(set)
    ...
    for label, path in named_paths:
        ...
        with path.open("r", encoding="utf-8", errors="replace") as f:
            for raw_line in f:
                ...
                seed_index[key].add(label)
                seeds_by_label[label].add(key)

    return seed_index, seeds_by_label


def discover_seed_players(
    match: Dict[str, Any],
    seed_index: Dict[str, set[str]],
    matched_seed_aliases: Dict[str, set[str]],
    tracked_player_sources: Dict[str, set[str]],
    alias_to_player_ids: Dict[str, set[str]],
) -> None:
    """
    Opportunistically resolve seed Riot IDs to stable hashed player IDs while
    the raw match is already in memory.

    Important: participant rows themselves do not need an is_seed flag. We
    produce a tiny tracked_players table and join it later by player_id. This
    avoids any problem where a seed alias is discovered only after earlier
    participant rows were already written.
    """
    if not seed_index:
        return
    ...
    for p in participants:
        ...
        if alias is None or alias not in seed_index:
            continue
        ...
        labels = seed_index[alias]

        alias_to_player_ids[alias].add(player_id)
        for label in labels:
            matched_seed_aliases[label].add(alias)
            tracked_player_sources[player_id].add(label)
...
def process(
    ...
    keep_puuid: bool,
    deduplicate: bool,
    seed_lists: List[Tuple[str, Path]],
) -> Dict[str, Any]:
    ...
    seed_index, seeds_by_label = load_seed_lists(seed_lists)
    matched_seed_aliases: Dict[str, set[str]] = defaultdict(set)
    tracked_player_sources: Dict[str, set[str]] = defaultdict(set)
    alias_to_player_ids: Dict[str, set[str]] = defaultdict(set)
    ...
    summary: Dict[str, Any] = {
        ...
        "seed_lists": [{"label": label, "path": str(path.resolve())} for label, path in seed_lists],
                ...
                try:
                    ...
                    for source_container, source_member, match in iterator:
                        ...
                        try:
                            ...
                            discover_seed_players(
                                match=match,
                                seed_index=seed_index,
                                matched_seed_aliases=matched_seed_aliases,
                                tracked_player_sources=tracked_player_sources,
                                alias_to_player_ids=alias_to_player_ids,
                            )
    ...
    # Write the small tracked-player lookup table once, after all aliases have
    # had a chance to resolve. No Riot display names are persisted.
    tracked_rows = [
        {
            "player_id": player_id,
            "seed_sources": ";".join(sorted(labels)),
        }
        for player_id, labels in sorted(tracked_player_sources.items())
    ]
    writer.write("tracked_players", tracked_rows)

    ambiguous_aliases = {
        alias: player_ids
        for alias, player_ids in alias_to_player_ids.items()
        if len(player_ids) > 1
    }

    seed_resolution_summary = {
        "method": (
            "Exact normalized Riot-ID alias matching (NFKC + casefold) while scanning raw "
            "Match-V5 participant records; resolved aliases are converted to hashed player_id."
        ),
        "privacy_note": "Raw Riot display names are not written to processed analytical tables.",
        "lists": {},
        "unique_tracked_player_ids_resolved": len(tracked_player_sources),
        "ambiguous_seed_alias_count": len(ambiguous_aliases),
    }

    for label, seeds in sorted(seeds_by_label.items()):
        matched = matched_seed_aliases.get(label, set())
        seed_resolution_summary["lists"][label] = {
            "seed_aliases_in_file": len(seeds),
            "seed_aliases_matched_in_scanned_matches": len(matched),
            "seed_alias_match_percent": (
                100.0 * len(matched) / len(seeds) if seeds else None
            ),
        }

    with (output / "audit" / "seed_resolution_summary.json").open(
        "w", encoding="utf-8"
    ) as f:
        json.dump(seed_resolution_summary, f, indent=2, ensure_ascii=False)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            'Optional tracked-player lists in NAME=PATH form, e.g. '
            '"NA=.\\data\\Games of League of Legends\\summoners_na_used.txt". '
            "Resolved players are written to tracked_players/ without display names."
        ),
    )
    parser.add_argument(
        "--format",
